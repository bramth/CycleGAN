...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        labels_X = torch.ones(images_X.size(0)).cuda()
        ...
        labels_Y = torch.ones(images_Y.size(0)).cuda()
        #images_Y, labels_Y = utils.to_var(images_Y), utils.to_var(labels_Y).long().squeeze()
        
        labels_X_fake = torch.zeros(images_Y.size(0)).cuda()
        labels_X_real = torch.ones(images_Y.size(0)).cuda()
        
        labels_Y_fake = torch.zeros(images_X.size(0)).cuda()
        labels_Y_real = torch.ones(images_X.size(0)).cuda()
        ...
        output_Y = D_Y(images_Y)        
        D_X_loss = loss(output_X,labels_X)
        D_Y_loss = loss(output_Y,labels_Y)
        ...
        #print('DEBUG PRINT}}',output_X.size(),labels_X_fake.size(),images_X.size())
        #print('DEBUG PRINT}}',output_Y.size(),labels_Y_fake.size(),images_Y.size())
    
        D_X_loss = loss(output_X,labels_X_fake) # SIZE LABELS?
        ...
        D_Y_loss = loss(output_Y,labels_Y_fake)
        ...
        # Y TO X TO Y
        for gen in np.arange(2*train_G):
            ...
            g_loss = loss(output_X,labels_X_real)
    
            if opts.use_cycle_consistency_loss:
                reconstructed_Y = G_XtoY(fake_X)
                # 3. Compute the cycle consistency loss (the reconstruction loss)
                cycle_consistency_loss = loss(reconstructed_Y,images_Y)
            ...
            g_loss = loss(output_Y,labels_Y_real)
    
            if opts.use_cycle_consistency_loss:
                reconstructed_X = G_YtoX(fake_Y)
                # 3. Compute the cycle consistency loss (the reconstruction loss)
                cycle_consistency_loss = loss(reconstructed_X,images_X)
                g_loss += cycle_consistency_loss
    
            g_loss.backward()
            g_optimizer.step()
        
        train_G = int(g_loss/d_fake_loss)+1
