...
def training_loop(train_dataloader, opts):
    ...
    fixed_noise = sample_noise(16,opts.noise_size)  # batch_size x noise_size x 1 x 1
    ...
    for epoch in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            fake_labels = torch.zeros(batch[0].size(0)).cuda()
            real_labels = torch.ones(batch[0].size(0)).cuda()
            ...
            D_real_loss = loss(output,real_labels)
            
            # 2. Sample noise
            noise = sample_noise(batch[0].size(0),opts.noise_size) # sample_noise(opts.noise_size,1)
            # 3. Generate fake images from the noise
            fake_images = G(noise)

            # 4. Compute the discriminator loss on the fake images
            output = D(fake_images)
            D_fake_loss = loss(output,fake_labels)
            ...
            for gen in np.arange(20*train_I):
                ...
                output = D(fake_images)
                G_loss = loss_G(output,real_labels)            
                
    
                G_loss.backward()
                g_optimizer.step()
    
            train_I = int(G_loss/D_fake_loss)+1
            # Print the log info
